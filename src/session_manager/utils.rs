...
/// Basic HTML entity escaping for content inside raw-HTML blocks like <details>.
...
fn extract_text_from_item(item: &Value) -> Option<String> {
    ...
    // thinking: collapsible block, expanded by default
    if item_type == "thinking" {
        let text = item.get("thinking").and_then(Value::as_str).unwrap_or("");
        if text.is_empty() { return None; }
        return Some(format!(
            "\n<details open class=session-details>\n<summary>🤔 思考过程</summary>\n\n<div class=thinking-body>{}</div>\n\n</details>\n",
            html_escape(&text)
        ));
    }

    // tool_use: show tool name + collapsible input parameters
    if item_type == "tool_use" {
        let name = item.get("name").and_then(Value::as_str).unwrap_or("unknown");
        let mut result = format!("**调用工具: {name}**");
    ...
    // tool_result: extract nested content, mark errors
    if item_type == "tool_result" {
        let is_error = item.get("is_error").and_then(Value::as_bool).unwrap_or(false);
        let prefix = if is_error { "**输出 (错误):**\n" } else { "**输出:**\n" };
